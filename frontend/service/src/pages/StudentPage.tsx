...
function AppLoginScreen({ onNext }: { onNext: (student: StudentInfo) => void }) {
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#f7f9fc] px-6 pt-16 pb-10 flex flex-col justify-between">
      <div>
    ...
    <div className="h-full overflow-y-auto bg-[#f7f9fc] px-6 pt-16 pb-10 flex flex-col justify-between">
      <div>
        ...
        <button
          ...
          className="w-full flex items-start gap-3 bg-white border-2 rounded-2xl p-4 transition-all mb-6"
...
function HomeScreen({ student, onNavigate }: { student: StudentInfo | null; onNavigate: (s: Screen) => void }) {
  const [showNotif, setShowNotif] = useState(false)

  return (
    <div className="flex flex-col h-full overflow-y-auto bg-[#f7f9fc]">
      <div className="bg-white px-5 pt-12 pb-5 border-b border-gray-100">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-400 font-medium tracking-wide">
              {student?.universityName || "숭실대학교"} · {student?.major || "소프트웨어학부"}
            </p>
      ...
      <div className="px-5 py-6 space-y-5">
        ...
        >
          <div className="w-12 h-12 rounded-xl bg-brand-600 flex items-center justify-center shrink-0 text-white font-bold">
            QR
          </div>
...
function HistoryScreen({ onBack, studentId }: { onBack?: () => void; studentId: number }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-[#f7f9fc]">
      ...
      <div className="px-5 py-5 space-y-3 pb-6">
...
function HospitalsScreen({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-[#f7f9fc]">
      ...
      <div className="px-5 py-5 space-y-2.5 pb-6">
        {filtered.map((h) => (
          <div
            key={h.code}
...
function ProfileScreen({ student, onBack }: { student: StudentInfo | null; onBack?: () => void }) {
    ...
    <div className="flex flex-col h-full overflow-y-auto bg-[#f7f9fc]">
      ...
      <div className="px-5 py-5 space-y-4 pb-6">
        <div className="bg-white rounded-2xl p-5 border border-gray-100">
          <p className="font-bold text-gray-900">{student?.name || "학생"}</p>

          <p className="text-xs text-gray-400 mt-1">
            {student?.studentId || "20231234"} · {student?.major || "소프트웨어학부"}
          </p>

          <p className="text-xs text-gray-400">{student?.universityName || "숭실대학교"}</p>
        </div>
...
export default function StudentPage() {
  ...
  const renderContent = () => {
    ...
    switch (tab) {
      case "home":
        return <HomeScreen student={student} onNavigate={handleNavigate} />
      ...
      case "profile":
        return <ProfileScreen student={student} onBack={goHome} />
    }
  }

  return (
    <div className="w-screen h-screen flex items-center justify-center bg-gray-200 font-sans overflow-hidden">
      <div className="relative w-full max-w-[390px] h-full max-h-[844px] bg-[#f7f9fc] overflow-hidden flex flex-col shadow-2xl rounded-[40px] border border-gray-300">
        ...
        {authStep === "app" && (screen === "home" || screen === "detail") && (
          <div className="bg-white border-t border-gray-100 px-2 pt-2 pb-4 flex items-center shrink-0">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => {
