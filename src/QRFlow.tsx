...
import { createVisit, getVisit, verifyQrToken, verifyStudent, type QrContext, type Visit } from "./api";
...
/* ─── Screen 1: Landing ─── */
function LandingScreen({ onNext, context }: { onNext: () => void; context: QrContext }) {
  return (
    <div className="flex flex-col min-h-full bg-white">
      ...
      <div className="bg-gradient-to-b from-brand-50 to-white px-6 pt-14 pb-8 text-center border-b border-gray-100">
        ...
        <h2 className="text-lg font-bold text-gray-900">{context.hospitalName}</h2>
        <p className="text-sm text-gray-500 mt-1">AutoMedi Partner Hospital</p>
...
/* ─── Screen 2: Login ─── */
function LoginScreen({ onVerify }: { onVerify: (studentNumber: string, name: string) => Promise<void> }) {
...
/* ─── Screen 3: Consent ─── */
function ConsentScreen({ onSubmit, context }: { onSubmit: () => Promise<void>; context: QrContext }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "접수에 실패했습니다.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const shared = [
    { label: "진료일자", value: new Date().toLocaleDateString("ko-KR"), shared: true },
    { label: "병원명", value: context.hospitalName, shared: true },
...
/* ─── Screen 5: Complete ─── */
function CompleteScreen({ onClose, visit }: { onClose: () => void; visit: Visit }) {
    ...
    <div className="flex flex-col min-h-full bg-[#f7f9fc]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-8">
        ...
        >
          ...
          {[
            { label: "병원명", value: visit.hospitalName },
...
export default function QRFlow({
  ...
  useEffect(() => {
    ...
    verifyQrToken(activeToken)
      .then(setContext)
      .catch((requestError: unknown) => {
        setError(requestError instanceof Error ? requestError.message : "QR을 확인할 수 없습니다.");
      })
      .finally(() => setLoading(false));
  }, [activeToken]);

  useEffect(() => {
    if (step !== "waiting" || !visit) return;
  ...
  const handleVerifyStudent = async (studentNumber: string, name: string) => {
    if (!activeToken) throw new Error("QR 토큰이 없습니다.");
    const response = await verifyStudent(activeToken, studentNumber, name);
  ...
  const handleDetected = (data: { token?: string }) => {
    if (!data.token) {
      setScanning(false);
      setError("AutoMedi 병원 QR이 아닙니다.");
      return;
    }
    setError("");
    setLoading(true);
  ...
  if (scanning) {
    return <QRScanner onBack={onClose} onDetected={handleDetected} />;
  }

  if (loading) {
    return <div className="size-full flex items-center justify-center bg-white text-sm text-gray-500">병원 QR을 확인하는 중…</div>;
  }

  if (error || !context) {
    return (
      <div className="size-full flex flex-col items-center justify-center gap-5 bg-white px-8 text-center">
        <div className="text-5xl">!</div>
        <div><h1 className="text-xl font-bold text-gray-900">QR을 사용할 수 없습니다</h1><p className="mt-2 text-sm text-gray-500">{error}</p></div>
        <button onClick={onClose} className="rounded-xl bg-brand-600 px-6 py-3 text-sm font-bold text-white">돌아가기</button>
      </div>
    );
  }
